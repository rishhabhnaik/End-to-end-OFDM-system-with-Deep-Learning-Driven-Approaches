...
def main():
    ...
    for SNRdb in SNR_values:
        ...
        equalized_Hest = OFDM_system.OFDM_equalize(OFDM_Frame_demod, H_est_matrix)# OFDM_system.H_exact)
        ...
        data.append((SNRdb, np.sum(abs(bits_transmitted-bits_est))/len(bits_transmitted)))
    ...
    print(data)
    savemat(f"SNR_plot_data_for_LSE.mat", {"Data":data})
